...
| 白板 / 画布 | 接入 `CanvasTool` 与本地交互画布；支持自动布局、缩放拖拽、节点编辑保存、产物联动、同步到 PPT 和 SVG / PNG / JSON 导出，AFFiNE 作为后续真实协同方向。 |
...
2. 任务完成后切到“画布”，拖拽节点或修改节点名称、说明。
3. 点击“同步到 PPT”，再切回 PPT 预览或下载 `.pptx`，确认已插入或替换最新画布结构说明页。
...
| 白板 / 画布 | 部分完成 | 已支持本地交互画布、自动布局、节点拖拽编辑、编辑保存、节点联动、同步到 PPT 和导出；AFFiNE 真实协同体验仍需完善。 |
...
> 说明：当前画布编辑已经可以手动同步到 PPT，系统会把最新画布结构插入或替换为一页结构说明并重新导出 `.pptx`。后续还需要做更细粒度的自动页内图形重绘和真实协同编辑。
...
- [x] CanvasTool、本地交互画布、自动布局、节点拖拽编辑、编辑保存、节点联动、同步到 PPT 和 SVG / PNG / JSON 导出。
