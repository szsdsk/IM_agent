...
| 白板 / 画布 | 接入 `CanvasTool` 与本地交互画布；支持自动布局、缩放拖拽、节点编辑保存、产物联动和 SVG / PNG / JSON 导出，AFFiNE 作为后续真实协同方向。 |
...
| 白板 / 画布 | 部分完成 | 已支持本地交互画布、自动布局、节点拖拽编辑、编辑保存、节点联动和导出；AFFiNE 真实协同体验仍需完善。 |
...
> 说明：当前画布编辑会保存到任务结果并同步给网页端，但还不会自动反向修改已生成的 PPT。后续需要补“画布变更 -> 重建 DeckSpec -> 局部重导出 PPT”的联动闭环。
...
- [x] CanvasTool、本地交互画布、自动布局、节点拖拽编辑、编辑保存、节点联动和 SVG / PNG / JSON 导出。
...
- [ ] AFFiNE 真实协同编辑体验。
- [ ] 画布编辑联动 PPT：根据画布节点/结构变化自动更新 DeckSpec，并重新生成或局部修订 PPT。
