...
| PPT 生成 | 生成 DeckSpec/页面数据，支持多主题、多布局和演示场景适配，并导出 `.pptx` 文件 |
| 语音识别 | 前端录音转 PCM16k，后端调用飞书 ASR；飞书 Bot 语音消息也可转写后触发 Agent |
| 飞书交付 | 基于飞书 OpenAPI 发送进度卡片、交付卡片、文本消息和 PPT 文件 |
| 网页工作台 | 三栏界面：左侧输入和场景选择，中间流程进度，右侧文档/PPT 预览 |
| 状态同步 | WebSocket 推送任务进度，SyncService 提供内存/Redis 两种同步通道 |
...
| 多端协同与一致性 | 部分完成 | 网页端 WebSocket、飞书 Bot 和 SyncService 已具备实时状态推送；尚未实现独立移动端客户端、离线编辑和冲突合并 |
| 白板/自由画布 | 未完成 | 当前主链路选择 PPT 方向，尚未接入白板或自由画布 |
| PPT 演练与修改闭环 | 未完成 | 已有卡片确认/修改入口和反馈处理基础，但演练稿、Q&A 训练、逐页改稿还未形成完整闭环 |
...
- [ ] 白板/自由画布生成和编辑
- [ ] PPT 逐页反馈修改、演练稿和 Q&A 训练闭环
- [ ] 更完整的多端协同：独立移动端、跨端编辑一致性、离线编辑和冲突合并
- [ ] 飞书文档内容级增量同步和版本差异展示
- [ ] 权限、审批、团队协作治理和交付归档策略
- [ ] 更完整的自动化测试、部署文档和演示脚本
