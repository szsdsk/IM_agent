...
| PPT 生成 | 生成 DeckSpec/页面数据，支持 3 套主题 + 4 种布局，导出本地 `.pptx` 文件 |
| 飞书交互卡片 | 进度卡片 + 交付卡片（含确认/修改按钮），替代纯文本消息 |
| 网页端 | 支持会话输入、进度查看、文档预览、PPT 预览、本地 PPT 下载、飞书文档编辑入口 |
...
| PPT 生成与导出 | 已完成 | 支持多主题多布局，导出带视觉效果的 `.pptx` 文件 |
...
| 飞书或类似 OpenAPI 集成 | 已完成 | bot 消息收发、交互卡片、文件上传/发送、云文档写入已接入 |
